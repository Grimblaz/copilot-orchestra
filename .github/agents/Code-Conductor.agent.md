...
Read the `Class` value (`[auto]` or `[manual]`) from the plan's `[CE GATE]` step scenario entries (e.g., `S1: {description} [auto]`). When BDD is enabled, replace the "Scenarios exercised (brief list)" with the per-scenario coverage table below:
