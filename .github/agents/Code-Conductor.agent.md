...
3. **BDD Phase 2 Runner Dispatch** (conditional — skip entirely when Phase 2 is not active; Phase 2 requires `## BDD Framework` heading AND `bdd: {framework}` line with recognized framework in consumer repo's `copilot-instructions.md`):
   1. **Phase 2 detection**: read `bdd: {framework}` from consumer `copilot-instructions.md`. Missing heading, heading-only without `bdd:` line, `bdd: true`, or unrecognized framework → skip this step entirely, proceed to step 4 with all scenarios (Phase 1 behavior unchanged).
   ...
   5. **Evidence merge**: combine runner evidence (for `[auto]`) with EO evidence when available (for `[manual]`). Same-scenario conflict (runner-pass + EO-fail or vice versa) → `source: runner+eo`, `result: conflict`.
...
| --- | ---------- | -------- | --------- | ------------------- | ----------- |
| S1  | Functional | [manual] | ✅ Passed | {brief description} | Runner      |
