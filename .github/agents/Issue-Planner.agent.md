...
- **Cross-file constants** — when a plan step (a) implements or modifies a script or module that consumes enumerated values produced by another file (stage names, category strings, enum labels), or (b) creates or modifies a file that authoritatively defines enumerated values consumed by scripts, the Requirement Contract must: (i) name the authoritative source file for those values (or identify all known consumer scripts via grep when the step modifies the producer), and (ii) list the exact allowed values as a quoted string enum in the AC (example format: `Allowed values: 'main' | 'postfix' | 'ce'`).
