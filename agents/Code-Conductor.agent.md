...
**Skip scope classification when**:

- Smart resume has confirmed full upstream completion — all three of `<!-- experience-owner-complete-{ID} -->`, `<!-- design-phase-complete-{ID} -->`, and `<!-- plan-issue-{ID} -->` are present in the issue comments. The tier and plan were established in a prior session; re-classifying is redundant. Proceed directly to the Hub Execution Workflow at step 4 (D9 Checkpoint). For multi-issue bundles, apply this skip only when ALL bundled issues satisfy the full-completion condition — any issue missing one or more of the three required markers still requires classification.
...
2. **Per-issue scope classification**: Apply the Scope Classification Gate skip condition first — if an issue already has all three full-upstream markers (`<!-- experience-owner-complete-{ID} -->`, `<!-- design-phase-complete-{ID} -->`, and `<!-- plan-issue-{ID} -->`), it is exempt from classification (tier already established as full). For issues that require classification, apply the Scope Classification Gate rubric independently. The bundle adopts the **highest-scope tier** (if any issue requires full pipeline, run full pipeline for all). Present all issue classifications (and any skip-exempt issues) in a single `#tool:vscode/askQuestions` call — do not make separate per-issue prompts. Format your recommendation as a list entry per issue showing recommended tier and the key criterion driving the classification, followed by the 'highest-scope-wins' bundle tier.
